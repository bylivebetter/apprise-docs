...
| -------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MEMORY` | In-memory only, no disk usage         | Ephemeral or restricted environments; This is the default for all plugins unless overriden with the `storage_mode` variable during its class initialzation |
...
- [**Jira**](https://github.com/caronc/apprise/blob/master/apprise/plugins/jira.py): Caches alert IDs so later `acknowledge`, `close`, `note`, and `delete` actions can find existing alerts without asking for the ID again.
...
- [**Opsgenie**](https://github.com/caronc/apprise/blob/master/apprise/plugins/opsgenie.py): Caches alert IDs so later `acknowledge`, `close`, `note`, and `delete` actions can find existing alerts without asking for the ID again.
...
- [**Email (PGP)**](https://github.com/caronc/apprise/blob/master/apprise/plugins/email/base.py): Uses persistent storage to hold auto-generated PGP key pairs. When PGP encryption is enabled without explicit key files, the plugin generates a public/private key pair on first use and writes both to the persistent storage path (`pgp-public.asc` and `pgp-private.asc`). Subsequent runs load the same keys from disk so every encrypted message is signed and encrypted with a stable, consistent identity.

Feel free to use these examples to help shape your own design or improve on a plugin that exists already that could really benefit from the Persistent Store.
